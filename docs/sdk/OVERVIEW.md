...
- ~~**Pagination.**~~ **Decided (2026-09-02, `yertle.nodes.list`).** The facade hides it: `list()` walks every page and returns one complete list. Callers want the graph, not page 1, and no caller so far has wanted otherwise. The loop stops on the reported `total` *or* a short/empty page, so a total that never catches up cannot spin. If a lazy variant is ever needed, add `nodes.iter()` beside `list()` rather than making every caller page by hand — do not change `list()`'s contract. Apply the same shape to `search` and any other paginating resource.
